...
Mode_Selector::Mode_Selector(QWidget* parent) :QWidget(parent), parent(parent)
{   
    ...
    QObject::connect(comboBox, QOverload<int>::of(&QComboBox::activated), this, &Mode_Selector::openModeWindow);
}


//This function opens or closes-and-reopens the menu with the appropriate widgets depending on the users's selection
void Mode_Selector::openModeWindow(int index) {
...
void Mode_Selector::safeDeleteAllWidgets(Container& container)
{   
...
void Mode_Selector::Mode1_DragDrop(QWidget* mainParentWidget, QVBoxLayout* layout) {
    ...
    QObject::connect(falseModeWidget, &FalseModeWidget::ModeNumber, this, &Mode_Selector::openModeWindow);
...
void Mode_Selector::Mode2_PathKey(QWidget* mainParentWidget, QVBoxLayout* layout)
{
    ...
    //Connects the filepath read from the enter button to Imagefromfilepath, transmitted image widget
    ...
    //Connects the filepath read from the Shift keyboard button to Imagefromfilepath, transmitted image widget
    QObject::connect(filePathWidget, &FilePathWidget::KeyPressed, imageFPathWidget, &ImageFPathWidget::PathSelected);
    QObject::connect(filePathWidget, &FilePathWidget::KeyPressed, grayFormMitWidget, &GrayFormMitWidget::recieveDroppedImg);
    QObject::connect(filePathWidget, &FilePathWidget::KeyPressed, ftransformWidget, &FTransformWidget::ImageDropped);
...
//Note: for a more detailed example on how these modes are creates in QTwidgets see Mode1
//      which is more heavy commented
void Mode_Selector::Mode3_SobelEdge(QWidget* mainParentWidget , QVBoxLayout* layout) 
{
    ...
    QHBoxLayout* dragTransLayout = new QHBoxLayout();
    DragDropWidget* dragDropWidget = new DragDropWidget(mainParentWidget);
    RgbMitwidget* rgbMitWidget = new RgbMitwidget(mainParentWidget);
    dragTransLayout->addWidget(dragDropWidget);
    dragTransLayout->addWidget(rgbMitWidget);
    modeLayout->addLayout(dragTransLayout);
    ...
    //connects the sobelEdgeWidget's generated hologram pattern to the RGB display/SLM transmission Widget
    QObject::connect(sobelEdgeWidget, &SobelEdgeWidget::holoPattern, rgbMitWidget, &RgbMitwidget::recieveHologramPattern);
